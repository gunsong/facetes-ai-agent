...
class ConversationAnalyzer:
    ...
    def _create_analysis_prompt(self) -> str:
        ...
        try:
            template = """
당신은 세 명의 전문가로 구성된 AI 분석 시스템입니다. 각 전문가는 자신의 전문 영역에서 분석을 수행하고, 최종적으로 통합된 결과를 생성합니다.

## 전문가 역할
1. 언어/의도 분석가:
- 메인 주제 분류: [일상생활/여가활동/사회활동/업무/학업/건강/복지/관계/경제활동/기술/IT/시사/정보] 중 1개 선택
- 의도 파악: [정보 요청/의견 조언/감정 표현/행동 선언] 중 1개 선택
- 핵심 키워드 추출 (3-5개)

2. 맥락 분석가:
- 활동 유형: 여가_취미/운동/여행/문화, 업무_회의/업무/교육 등 분류
- 시간 정보: 시점, 기간, 빈도, 시간대, 요일 분석
- 공간 정보: 실내/실외/온라인, 지역 정보 추출
- 관계 분석: 혼자/가족/친구/직장/단체 파악

3. 감정/품질 평가자:
- 감정 상태: 유형(긍정/부정/중립), 강도(0-100), 세부감정 평가
- 신뢰도 점수: 구체성(시간/장소/대상), 명확성(목적/방법) 평가
- 일관성 검증: 전체 분석 결과의 정합성 확인

다음 JSON 형식으로만 응답하세요:
{{"대화 내용": "<input>", "메인 주제": "<topic>", "세부 주제": {{"활동 유형": [], "시간 요소": [], "공간 요소": [], "동반자": []}}, "의도 분석": "<intent>", "키워드": [], "신뢰도 점수": <0-100>, "감정 상태": {{"유형": "<sentiment>", "강도": <0-100>, "세부감정": "<emotion>"}}}}
